    ...
    def __init__(self, argv=sys.argv[1:]):
        ...
        if 'refresh_token' not in SyncY.syncytoken or SyncY.syncytoken['refresh_token'] == '' or (len(self.__argv) != 0 and self.__argv[0] in ['sybind', 'cpbind']):
            ...
            if (('authurl' not in SyncY.syncytoken or SyncY.syncytoken['authurl'] == '') and len(self.__argv) == 0) or (len(self.__argv) != 0 and self.__argv[0] == 'sybind'):
                ...
                print('     1. Open web browser to visit:"https://openapi.baidu.com/oauth/2.0/authorize?client_id=&" and input user code to binding your baidu account.')
                print(' ')
                raw_input('     2. After granting access to the application, come back here and press [Enter] to continue.')
                print(' ')
            if len(self.__argv) != 0 and self.__argv[0] == 'cpbind':
                ...
                if 'auth_url' in bindinfo and 'authcode' in SyncY.config and SyncY.config['authcode'] != '':
                    SyncY.syncytoken['authurl'] = bindinfo['auth_url']
                else:
                    sys.exit(5)
            
            auth_code = SyncY.config['authcode']
            error_msg = None
            responses = None
            for auth in AuthServerList:
                (url, _, __) = auth
                params = {'code': auth_code, 'bypy_version': BYPY_VERSION, 'redirect_uri': 'oob'}
                retcode, responses = sycurl.request(url, params, {}, 'GET', SYCurl.Normal)
                retcode = int(retcode)

                if retcode != 200 or 'error_code' in responses:
                    error_msg = '%s ERROR(Errno:%d): Get token failed: %s.' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), retcode, responses['error_msg'])
                else:
